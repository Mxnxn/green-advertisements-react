    ...
    const AddClient = async () => {
        ...
        try {
            const res = await clientBackend.addClient({
                ...
                cname: client.cname.toUpperCase(),
                pname: client.pname.toUpperCase(),
                email: client.email.toUpperCase(),
                gst: client.gst.toUpperCase(),
    ...
    const updateClient = async () => {
        ...
        try {
            ...
            const res = await clientBackend.updateClient({
                cid: client.cid,
                password: client.password ? client.password.toUpperCase() : null,
                cname: client.cname.toUpperCase(),
                pname: client.pname.toUpperCase(),
                email: client.email ? client.email.toUpperCase() : "",
                address: client.address ? client.address.toUpperCase() : "",
                gst: client.gst,
            });
                            ...
                            <tbody>
                                {state.clients.map((elem, index) => (
                                    <tr key={index} class="table-light">
                                        ...
                                        <td>{elem.phone}</td>
                                        <td>{elem.password}</td>
                                        <td>{elem.email ? elem.email : "N/A"}</td>
                                        <td>{elem.gst ? elem.gst : "N/A"}</td>
                                        <td>
                                            {elem.hid.length > 0 ? (
                                                <span
                                                    className="hyperlink"
